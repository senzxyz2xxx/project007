...
// ─────────────────────────────────────────────
const SHOP_ITEMS = [
  {id:'potion',     name:'ยาฟื้น HP',       icon:'🧪', desc:'ฟื้น HP 30',         rarity:'common', price:30},
  {id:'bomb',       name:'ระเบิดดาเมจ',     icon:'💣', desc:'ดีลบอส 25 HP',       rarity:'rare',   price:70},
  {id:'scroll',     name:'คัมภีร์ข้ามด่าน', icon:'📜', desc:'ข้ามคำถาม 1 ข้อ',    rarity:'rare',   price:60},
  {id:'crystal',    name:'คริสตัลพลัง',     icon:'💎', desc:'XP x2',               rarity:'epic',   price:120},
  {id:'magnet',     name:'แม่เหล็กทอง',     icon:'🧲', desc:'Gold x2',             rarity:'rare',   price:80},
  {id:'mega_pot',   name:'ยาฟื้น HP Max',   icon:'💉', desc:'ฟื้น HP เต็ม',        rarity:'epic',   price:180},
  {id:'mega_bomb',  name:'ระเบิดนิวเคลียร์',icon:'☢️', desc:'ดีลบอส 60 HP',       rarity:'epic',   price:200},
  {id:'shield_item',name:'โล่เพชร',         icon:'🔷', desc:'ป้องกัน 3 ครั้ง',     rarity:'epic',   price:150},
];
...
function updateStatusBar() {
  ...
  const next   = xpForLevel(P.level + 1) || (P.level * 600 + 1000);
  const cur    = P.xp - base;
...
function addXP(amount) {
  P.xp += amount;
  while (P.level < 10 && P.xp >= xpForLevel(P.level + 1)) {
    P.level++;
    showLevelUpOverlay();
  }
  savePlayerData();
  ...
  SHOP_ITEMS.forEach(item => {
    ...
    div.innerHTML =
      `<div class="shop-icon">${item.icon}</div>
       ...
       <button class="btn-buy" ${canBuy?'':'disabled'} onclick="buyItem('${item.id}',${item.price})">
         ${canBuy?'ซื้อ':'Gold ไม่พอ'}
       </button>`;
...
function buyItem(id, price) {
  if (P.gold < price) { toast('Gold ไม่พอ!', 'red-toast'); return; }
  P.gold -= price;
  ...
  toast('🛒 ซื้อ '+item.name+' แล้ว!', 'green-toast');
}
...
function renderLeaderboard() {
  ...
  const rows = Object.values(users)
    .filter(u => !u.isAdmin)
    .map(u => ({ name:u.name, level:u.level||1, score:u.totalScore||0, wins:u.wins||0 }))
    .sort((a,b) => b.score - a.score)
    .slice(0, 20);

  if (!rows.length) { el.innerHTML = '<div class="text-muted" style="text-align:center;padding:20px">ยังไม่มีข้อมูล</div>'; return; }

  el.innerHTML = rows.map((r,i) => `
    <div class="board-row">
      <div class="board-rank ${i===0?'r1':i===1?'r2':i===2?'r3':''}">${i===0?'🥇':i===1?'🥈':i===2?'🥉':i+1}</div>
      <div class="board-name">${r.name}</div>
      <div class="board-stats">
        <div class="board-score">${r.score.toLocaleString()}</div>
        <div class="board-level">Lv.${r.level} | 🏆${r.wins} wins</div>
      </div>
    </div>`).join('');
}
